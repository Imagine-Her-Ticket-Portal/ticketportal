...
import { faCalendarAlt, faArrowDown, faFileDownload, faArrowRight, faRefresh, faMagnifyingGlass, faSearch } from "@fortawesome/free-solid-svg-icons";
import './AdminPreviewAllTickets.scss';
                            ...
                            <tbody>
                                {filteredTickets.map((ticket, index) => (
                                    <tr style={{ width: '100%' }} key={ticket._id}>
                                        ...
                                        <td>{ticket.raisedBy.location}</td>
                                        ...
                                        <td>{ticket.dateResolved === null ? 'Not resolved' : formatDate_dd_mm_yyyy(ticket.dateResolved)}</td>
                                        {
                                            ticket.raisedBy && (
                                                <>
                                                    ...
                                                    <td>{(ticket.raisedBy.businessName !== null && ticket.raisedBy.businessName.length !== 0) ? ticket.raisedBy.businessName : 'Not Added Yet'}</td>
                                                </>
                                            )
                                        }
                                        {
            ...
            <div className='row'>
                <div className="form-outline col-md-12 col-sm-12">
                    <input
                        ...
                        placeholder="Search By Ticket Status / Category / Sub-Category / Client Name / Location / Business Name"
                        aria-label="Search"
        ...
        const fetchTickets = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error(`Failed to fetch tickets: ${errorData.error}`);
                }
            }
            catch (err) {
                alert(err.message)
        ...
        <>
            <div className='preview-buttons-container'>
                <div className='all-tickets'>
                    <p style={{ borderBottom: '2px solid #D81F84', paddingBottom: '0.5rem' }} className={`show-all-tickets`}>All Tickets</p>
                </div>
            ...
            <SearchComponent searchParams={searchParams} setSearchParams={setSearchParams}/>
            {
                isAuthenticated && startDate.length > 0 && endDate.length > 0
                    ? (getSelectedTickets.length > 0
                        ? <TicketsTable searchParams={searchParams} showTickets={name} tickets={getSelectedTickets} setFilteredTicketsByParams={setFilteredTicketsByParams}/>
                        : <h3 style={{ margin: '50px', textAlign: 'center' }}>{message}</h3>)
                    : <TicketsTable searchParams={searchParams} showTickets={name} tickets={getTickets} setFilteredTicketsByParams={setFilteredTicketsByParams}/>
...
export default AdminPreviewAllTickets